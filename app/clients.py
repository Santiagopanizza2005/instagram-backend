...
class AccountManager:
    ...
    def login(self, username: str, password: str, verification_code: Optional[str] = None, webhook_url: Optional[str] = None):
        c = Client()
        # Configurar cliente automáticamente con IP y dispositivo del usuario
        auto_config.setup_instagram_client(c)
    ...
    def import_session(self, username: str, sessionid: str, webhook_url: Optional[str] = None):
        """Importar sesión de forma simplificada - sin cookies manuales"""
        c = Client()
        # Configurar cliente automáticamente
        auto_config.setup_instagram_client(c)
    ...
    def send_file(self, username: str, recipient: str, source: str, overrides: Optional[dict] = None):
        ...
        if isinstance(source, str) and source.lower().startswith("http"):
            limit = None
            try:
                limit = int(os.environ.get("MAX_UPLOAD_BYTES", "3000000"))
            except Exception:
                limit = 3000000
            ...
            try:
                ...
                if cl and limit and int(cl) > limit:
                    raise ValueError("file_too_large")
            except Exception:
                pass
            if limit and r.content is not None and len(r.content) > limit:
                raise ValueError("file_too_large")
            ext = None
            if ct:
